...
let displayTable=(books:any[],table) =>{
...
let displaySearchedBooks=(temp:any[]) =>{
  if(temp.length != 0)
  {
...
let searchByText=(text:string,searchby:string) =>{
  let searchedBooks=[];
  // let isthere:boolean;
...
let searchBooksByValue=(value:string,searchby:string) => {
  let searchedbooks =[];
  for(let i=0;i<books.length;i++)
  {
...
//Search text
let textSearcher=(fulltext,searchedtext) => {
